...
namespace AcmeBank
{
    class CurrentAccount : AccountService
    {
        ...
        public CurrentAccount(int accountId, string customerNum, int balance, int overdraft)
        {
        ...
        public void withdraw(int accountId, int amountToWithdraw)
        {
            try
            {
                ...
                else
                {
                    ...
                    if ((balanceOfthisAccount - amountToWithdraw) < 0)
                        throw new Exception();
                    else
                    {
                        SystemDB.Instance.SetBalance(accountId, amountToWithdraw - balanceOfthisAccount);
                        //decrease the overdraft amount here 
